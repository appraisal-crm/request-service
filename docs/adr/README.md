# Architecture Decision Records

> English version · [Русская версия](i18n/ru/README.md)

An ADR documents a significant architectural decision: what was decided, why, and what the trade-offs are.

## Index

| ADR | Title | Status |
|-----|-------|--------|
| [ADR-001](ADR-001-kafka.md) | Apache Kafka as Message Broker | Proposed |
| [ADR-002](ADR-002-keycloak.md) | Keycloak for Authentication and Authorization | Proposed |
| [ADR-003](ADR-003-database-per-service.md) | Database per Service | Proposed |
| [ADR-004](ADR-004-open-source-license.md) | Open Source License (MIT / Apache 2.0) | Proposed |
| [ADR-005](ADR-005-go-backend.md) | Go for Backend Services | Proposed |
| [ADR-006](ADR-006-s3-storage.md) | S3 Yandex Cloud for File Storage | Proposed |
| [ADR-007](ADR-007-event-delivery-outbox.md) | Reliable Event Publishing via Transactional Outbox | Proposed |

## Statuses

- **Proposed** — under discussion
- **Proposed** — decision made and in effect
- **Deprecated** — no longer relevant
- **Superseded** — replaced by a newer ADR
